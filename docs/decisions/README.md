...
| [034](ADR-034-the-grid-is-a-hypothesis-set-not-a-search-space.md) | A grade de parâmetros é **conjunto de hipóteses** (um campo por candidato, com a pergunta escrita ao lado), nunca produto cartesiano; treino ordena, validação escolhe, **teste só reporta**; e a figura que responde é a **degradação**, não o retorno | Accepted | walk-forward, overfitting, out-of-sample, rule 60/61/62, W14 |
| [035](ADR-035-equal-segments-from-an-empty-portfolio.md) | Os três segmentos têm o **mesmo tamanho** e cada um parte de **carteira vazia** — a idade da carteira é confundidor da própria comparação, removido por construção; e quem não preencheu ordem nenhuma **não é ranqueado** em vez de pontuado em zero | Accepted | walk-forward, janelas móveis, comparabilidade, rule 61/62, irmão do ADR-034 |
| [036](ADR-036-the-request-window-is-part-of-a-bars-identity.md) | A **janela do pedido** faz parte da identidade da barra intraday: dois baldes particionam a mesma sessão de formas incompatíveis (0 de 135 barras idênticas entre `5d` e `3mo`, contra 135 de 135 no mesmo balde), então a sessão vem inteira de uma janela só e a mistura é **reportada, nunca resolvida em silêncio** | Accepted | intraday, ingestão idempotente, rule 19/20/47, W15, irmão do ADR-020 |
| [037](ADR-037-a-gap-is-measured-a-session-edge-is-compared.md) | **Buraco se mede** (entre barras entregues, dentro da sessão); **borda de sessão se compara** com as vizinhas do lote, nunca com um horário de pregão presumido. Sem verificação de alinhamento de grade — o dado real tem sessão fora de fase — e fuso da bolsa como offset fixo | Accepted | intraday, data quality, gaps, timezone, rule 18/20, W15 |
