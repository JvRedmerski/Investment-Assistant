...
| POST | `/{ticker}/intraday/sync` | ingere velas intraday; query `timeframe` (`1m`/`5m`/`15m`, default `15m`), `days` (1–90, default 5) e `resync`. **`days` em vez de `start`/`end` porque a fonte só funciona assim**: todo range dela é ancorado no instante do pedido e não aceita data inicial. Resposta: `fetched/inserted/skipped_existing/rejected/replaced/missing_bars`, mais `window`, `sessions[]` e `conflicts[]`. ⚠️ **`conflicts` não vazio é sucesso**, não falha — são as sessões já gravadas sob **outra janela**, que a ingestão deixa intactas porque duas janelas particionam a sessão de formas incompatíveis ([ADR-036](../decisions/ADR-036-the-request-window-is-part-of-a-bars-identity.md)); `resync=true` substitui a sessão inteira. Fonte que não serve o ticker → **400 `INTRADAY_NOT_AVAILABLE`** (não 503: é limite de plano, e nenhum retry resolve) |
| GET | `/{ticker}/intraday` | lê **só** do banco; query `timeframe` e `days`. Devolve **envelope**, não lista: `{ticker, timeframe, windows[], session_count, bars[]}`. `windows` com mais de uma entrada significa que a série tem uma **emenda** entre sessões — a ingestão garante que nenhuma *sessão* mistura janelas, não que uma série de várias sessões seja homogênea —, e qualquer cálculo que atravesse fronteira de sessão está lendo através dela |

Os endpoints `*/sync` são as únicas rotas que chamam provedores externos. `indicators/compute` escreve no banco mas não faz I/O de rede.

⚠️ **O universo intraday é menor que o de preço diário.** No plano gratuito da Brapi o acesso intraday é liberado **por ticker**: PETR4, ITUB4, MGLU3 e VALE3 são servidos; BBAS3 e BOVA11 respondem HTTP 400. A mensagem do fornecedor culpa o *intervalo*, e o intervalo é o mesmo nos dois grupos. No caminho intraday um ticker **inexistente** devolve o mesmo erro, nunca 404 — o gate de plano roda antes da resolução do ticker —, então `INTRADAY_NOT_AVAILABLE` **não** pode ser lido como "ticker não existe".
