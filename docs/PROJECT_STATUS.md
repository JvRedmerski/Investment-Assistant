...
## Current Phase
- **Phase**: entre waves — a wave **PRICE** (histórico de preços de fonte aberta, B3 COTAHIST) fechou em 2026-08-19
- **Status**: 🟢 PRICE COMPLETED (3 tasks), wave **inserida fora da ordem do roadmap** entre a W09 e a W10, por decisão do usuário. Próxima wave ainda não iniciada; a escolha está em [memory/CURRENT_TASK.md](memory/CURRENT_TASK.md)
...
- **Database**: PostgreSQL 16 + SQLAlchemy 2.0 Models + Alembic (`001` … `010_nullable_adj_close`) (`backend/app/data/models`) 🟢 COMPLETED
...
Wave: — (entre waves)
Task ID: —
Status: ⚪ aguardando escolha da próxima wave

> Esta seção estava parada em "Wave 08 / W08-001 ⚪ NOT_STARTED" desde antes de a W08 ser
> entregue; a lista abaixo também parava na W07. Corrigido em 2026-08-19, junto com o registro
> da wave PRICE. O detalhe task-a-task fica em *Last Execution* e em
> [history/COMPLETED_TASKS.md](history/COMPLETED_TASKS.md).
...
Next Action:
Escolher entre **eventos societários e proventos** (recomendado — fecha `dy`, o pilar de Risco,
a cobertura do score e o backtesting da W13 de uma vez) e a **Wave 10 — Rebalanceamento**
(a ordem do roadmap). Ver `docs/memory/CURRENT_TASK.md`.
...
## In Progress
Nenhuma task em andamento. **Wave PRICE concluída**; a próxima está em decisão — ver [memory/CURRENT_TASK.md](memory/CURRENT_TASK.md).
...
## Future Work
- **`dy` a partir da DMVL/DMPL da CVM**: dividendos e JCP debitados ao patrimônio no exercício estão em `5.04.06` e `5.04.07` (contas fixas), coluna `Patrimônio Líquido`. Medido no arquivo de 2024, a PETR4 dá R$ 100,9 bi, ~R$ 7,83 por ação. Ficou fora da W09-003 porque **nenhum pilar de score consome `dy` hoje** (regra 134); entra quando houver tela ou pilar que o use.
- **Histórico de preços é o que ainda trava `pe`/`pb` no banco real.** A contagem de ações chegou, mas `_price_on_or_before` exige preço na data de referência ou antes, e o teto de 3 meses da Brapi não alcança nenhum fechamento de exercício passado. A fonte natural é o COTAHIST da B3 — dado aberto, mesma natureza dos arquivos da CVM. É a mesma dependência que trava a W13.
...
## Next Action
**Eventos societários e proventos** (recomendado): uma ingestão fecha `dy` (o último dos 10 indicadores ainda `None`), o **pilar de Risco**, a cobertura do score (0,75 → 1,00) e o backtesting da W13. O preço bruto já está no banco; a série ajustada é derivável dele mais os eventos. Alternativa: **Wave 10 — Rebalanceamento** (roadmap §22, AGENTS.md §34). Ver `docs/memory/CURRENT_TASK.md`.
