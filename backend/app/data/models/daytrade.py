...
class DayTradeSetup(Base):
    ...
    strategy: Mapped[str] = mapped_column(String(50), nullable=False)  # BREAKOUT, PULLBACK, VWAP
    ...
    status: Mapped[str] = mapped_column(String(20), default="DETECTED", nullable=False)  # DETECTED, TRIGGERED, EXPIRED, CLOSED
    reason: Mapped[str] = mapped_column(Text, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, nullable=False)

    asset: Mapped["Asset"] = relationship("Asset")
    result: Mapped[Optional["DayTradeResult"]] = relationship("DayTradeResult", back_populates="setup", uselist=False, cascade="all, delete-orphan")
...
class DayTradeResult(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    setup_id: Mapped[int] = mapped_column(Integer, ForeignKey("daytrade_setups.id", ondelete="CASCADE"), unique=True, nullable=False)
    exit_price: Mapped[float] = mapped_column(Float, nullable=False)
    exit_timestamp: Mapped[datetime] = mapped_column(DateTime, nullable=False)
    result: Mapped[str] = mapped_column(String(20), nullable=False)  # WIN, LOSS, BREAKEVEN
    ...
    setup: Mapped["DayTradeSetup"] = relationship("DayTradeSetup", back_populates="result")
